...
class Testbed(Resource):
    ...
    def to_dict(self, head_only = False):
        ...
        if self.id == 'TWIST':
            r['background_image'] = build_url(server_url = self.server_url, path = '/uploads/testbed/background.jpg')
            ...
            r['coordinates_mapping_function_x'] = None
            r['coordinates_mapping_function_y'] = None
        return r
    ...
    def to_dict(self, head_only = False):
        ...
        if not head_only:
            ...
            if self.experiment:
                ...
                r['experiment'] = None
            r['tasks'] = sorted([ t.to_dict(head_only = True) for t in self.tasks.all() ], key=lambda k: k['step'])
            r['datetime_created'] = utc_datetime_to_utc_string(self.datetime_created)
            r['datetime_modified'] = utc_datetime_to_utc_string(self.datetime_modified)
        return r
...
class Task(Resource):
    ...
    def get_absolute_url(self):
        return '/jobs/%s/tasks/%s' % (self.job.id, self.id)
