...
class COTEFEClientError(Exception):
...
class COTEFEAPIError(Exception):
...
def bind_method(**config):
    ...
    class COTEFEAPIMethod(object):
        ...
        def _build_parameters(self, args, kwargs):
            ...
            for index, value in enumerate(args):
                ...
                try:
                    ...
                    raise COTEFEClientError("Too many arguments supplied")

            for key, value in kwargs.iteritems():
                if value is None:
                    continue
                if key in self.parameters:
                    raise COTEFEClientError("Parameter %s already supplied" % key)
        ...
        def _do_api_request(self, url, method="GET", body=None, headers={}):
            ...
            if response['status'] == '503':
                raise COTEFEAPIError(response['status'], "Rate limited", "Your client is making too many request per second")
            ...
            if status_code == 200:
                ...
                raise COTEFEAPIError(status_code, content_obj['meta']['error_type'], content_obj['meta']['error_message'])
    ...
    def _call(api, *args, **kwargs):
        method = COTEFEAPIMethod(api, *args, **kwargs)
