...
class Image(Resource):
    ...
    def get_absolute_url(self):
        return '%s/%s/%s' % (config.SERVER_URL, 'image', self.id)
    ...
    def to_dict(self, head_only = False):
        ...
        if not head_only:
            ...
            if self.file:
                r['download'] = '%s/uploads/%s' % (config.FEDERATION_SERVER_URL,  self.file.name)
            else:
                r['download'] = None
            r['datetime_created'] = utils.local_datetime_to_utc_string(self.datetime_created)
            r['datetime_modified'] = utils.local_datetime_to_utc_string(self.datetime_modified)
...
class NodeGroup(Resource):
    ...
    nodes = models.ManyToManyField(Node, related_name='nodegroups')
    image = models.ForeignKey(Image, null=True, blank=True)
    datetime_created = models.DateTimeField(auto_now_add=True)
    datetime_modified = models.DateTimeField(auto_now=True)
